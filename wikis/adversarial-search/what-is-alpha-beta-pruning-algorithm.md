...
Different from the minimax algorithm nodes, nodes in alpha-beta pruning algorithms will be assigned two values --- $$\alpha$$ value and $$\beta$$ value. α value is the lower bound utility value that can be found so far and $$\beta$$ value is the upper bound utility value that can be found so far. 
